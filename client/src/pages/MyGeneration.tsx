...
const MyGeneration = () => {
  ...
  return (
    <>
    <SoftBackdrop />
    <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24">
      {/* Header */}
      <div className="mb-8">
        ...
        && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
            {thumbnails.map((thumb: IThumbnail) => {
              const aspectClass = aspectRatioMap[thumb.aspect_ratio || '16:9'];
              return(
                <div key={thumb._id} onClick={() => navigation(`/generate/${thumb._id}`)} className='mb-8 group relative cursor-pointer break-inside-avoid rounded-2xl bg-white/6 border border-white/10'>
                  {/* Image */}
                  <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                    ...
                    {thumb.isGenerating && <div className="absolute inset-0 flex items-center justify-center bg-black/50 text-sm font-medium text-white">Generating...</div>}
